...
| `intermissionDuration` | `2` (seconds to show a static loading screen between modes; values below the two-second floor are clamped, and the screen remains until the next mode's background refresh finishes)
